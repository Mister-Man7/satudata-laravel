...
<div
    {{ $attributes->merge(['class' => 'group overflow-hidden rounded-3xl border border-gray-200 bg-white p-5 shadow-sm transition hover:shadow-lg sm:p-6 md:p-8']) }}>
    <div class="flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
        <div class="w-full flex-1 text-center lg:max-w-2xl lg:text-left">
            <div
                class="inline-flex items-center rounded-full bg-blue-50 px-4 py-2 text-xs font-bold tracking-wide text-blue-600 ring-1 ring-blue-100 sm:text-sm">
                {{ $badge }}
            </div>
            <h3 class="mt-5 text-2xl font-bold tracking-tight text-gray-900 sm:text-3xl lg:text-4xl">{{ $title }}
            </h3>

            @if ($description)
                <p class="mx-auto mt-3 max-w-xl text-sm leading-6 text-gray-500 sm:text-base lg:mx-0">{{ $description }}
                </p>
            @endif
